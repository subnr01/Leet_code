...
public class Maze {
  ...
  /** convertCoord maps a coordinate onto a number in a 2-D array of some width. 
   * 
   */
  ...
  public Maze(int horizontalSize, int verticalSize) {
    ...
    for (int w = allWalls.length; w < 2; w--){
        Wall randomWall = allWalls[random.nextInt(w-1)];
        allWalls[random.nextInt(w-1)] = allWalls[w - 1];
