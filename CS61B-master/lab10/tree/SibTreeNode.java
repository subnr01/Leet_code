  ...
  public void insertChild(Object item, int c) throws InvalidNodeException {
    // FILL IN YOUR SOLUTION TO PART II HERE.
      ...
      if(!isValidNode()){
          ...
          SibTreeNode index = (SibTreeNode) this.child(c);

          
              if(index.isValidNode()){
              
                  if(c == this.children()){
                      index.nextSibling = newChild;
                  }
                  else{
                      newChild.nextSibling = index.nextSibling;
                      index.nextSibling = newChild;
                  }
              } else{
...
/*
      ...
      int numChild;
      if (c > this.children()){
         numChild = this.children() + 1;
      }else{
          numChild = c;
      }
      if (this.firstChild.isValidNode()){
      SibTreeNode index = this.firstChild;
      int count = 2;
      while (count < numChild){
          if (index.nextSibling.isValidNode()){
      ...
      newChild.parent = this;
      if (index.nextSibling.isValidNode()){
      SibTreeNode tempSibling = index.nextSibling;
      }else{
          SibTreeNode tempSibling = null;
      index.nextSibling = newChild;
      newChild.nextSibling = tempSibling;
      myTree.size++;
      }
      }else{
